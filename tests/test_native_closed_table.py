"""Tests for Tabula._check_open — using a table after close().
...
character instead of erroring). Every Tabula method that touches
...
def test_add_row_after_close_raises(closed_table):
    with pytest.raises(native.TabulaError, match="closed"):
